...
export function CareersPage() {
    ...
    return <PageFrame>
        ...
        <section className="section section--careers"><div className="shell careers-layout"><Reveal className="careers-copy"><Eyebrow>Make an introduction</Eyebrow><h2>No performative<br /><span>hoops.</span></h2><p>Share enough for us to understand your point of view. A portfolio, a profile, or a clear note is often more useful than a perfect cover letter.</p><div className="careers-list"><span><Check size={15} /> Product & design</span><span><Check size={15} /> Engineering</span><span><Check size={15} /> Operations & growth</span></div><a className="email-link" href="mailto:contact@buildwithsravan.dev">Prefer email? contact@buildwithsravan.dev <ArrowUpRight size={15} /></a></Reveal><Reveal className="form-card reveal-delay-1">{submitted ? <div className="form-success"><CheckCircle2 size={30} /><h3>Thanks for reaching out.</h3><p>Your introduction is in the right place. We’ll take a considered look and get back to you.</p><ButtonLink to="/" variant="secondary">Back to YRecall <ArrowUpRight size={15} /></ButtonLink></div> : <form onSubmit={handleSubmit}><div className="form-card__header"><span>CAREER INTRODUCTION / 01</span><small>Fields marked * are required</small></div><div className="form-row"><label>Full name *<input required type="text" placeholder="Your name" /></label><label>Email address *<input required type="email" placeholder="you@email.com" /></label></div><label>Area of interest *<select required defaultValue=""><option value="" disabled>Choose an area</option><option>Product & design</option><option>Engineering</option><option>Operations & growth</option><option>Something else</option></select></label><label>What would you like to build with us? *<textarea required rows={4} placeholder="A short note about your work, your curiosity, or the problem space you care about." /></label><div className="form-row"><label>Portfolio URL<input type="url" placeholder="https://" /></label><label>LinkedIn profile<input type="url" placeholder="https://linkedin.com/in/" /></label></div><label className="file-field"><span>Resume or work sample</span><span className="file-drop"><Upload size={17} /> Choose a file <small>PDF, DOCX up to 10MB</small><input type="file" accept=".pdf,.doc,.docx" /></span></label><button className="button button--primary form-submit" type="submit">Send introduction <Send size={16} /></button></form>}</Reveal></div></section>
    </PageFrame>;
}
...
function SupportForm({ bug = false }: { bug?: boolean }) {
    ...
    return <form onSubmit={handleSubmit}><div className="form-card__header"><span>{bug ? "BUG REPORT / 02" : "CONTACT DESK / 01"}</span><small>We usually reply within 2 business days</small></div><div className="form-row"><label>Your name *<input required type="text" placeholder="Your name" /></label><label>Email address *<input required type="email" placeholder="you@email.com" /></label></div>{bug ? <><label>What happened? *<select required defaultValue=""><option value="" disabled>Choose a category</option><option>Something is not saving</option><option>I cannot sign in</option><option>Something looks wrong</option><option>Other</option></select></label><label>Tell us what you saw *<textarea required rows={5} placeholder="What were you trying to do? What happened instead?" /></label><label className="file-field"><span>Attach a screenshot</span><span className="file-drop"><Paperclip size={17} /> Add an image <small>PNG, JPG up to 10MB</small><input type="file" accept="image/png,image/jpeg" /></span></label></> : <><label>How can we help? *<select required defaultValue=""><option value="" disabled>Choose a topic</option><option>Getting started</option><option>Account or access</option><option>Product feedback</option><option>Partnerships</option><option>Something else</option></select></label><label>Message *<textarea required rows={5} placeholder="Tell us a little about what you need." /></label></>}<button className="button button--primary form-submit" type="submit">{bug ? "Send bug report" : "Send message"} <Send size={16} /></button></form>;
}
